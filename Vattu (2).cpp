...
{  bool i= true;
  do{
	...
	ThemNode(ds.ds,vt);   
	ds.n++;
	cout<<"\nBan muon nhap them nhan vien. Y:N?";
    getline(cin,c);
	if(strcmpi(c.c_str(),"Y")==0){
	 i= true;
	}
	else if(strcmpi(c.c_str(),"N")==0)
	{
		i= false;
	}
	}while(i==true);
...
void DiTimNodeThayThe(Tree &X, Tree &Y) 
{
	...
	else 
	{
		...
		X = Y; // cho node X(là node mà chúng ta s? di xóa sau này) tr? d?n node th? m?ng ==> ra kh?i hàm th? ta s? xóa node X
				...
				for(int j=0;j<hd->dscthd.n;j++)
					{
						...
						{
							*checkdulieu=true;
							sum[hd->ngaylap.month-1]+=(hd->dscthd.dshd[j].dongia*hd->dscthd.dshd[j].soluong)+(hd->dscthd.dshd[j].dongia*hd->dscthd.dshd[j].soluong*(hd->dscthd.dshd[j].vat/100));
